...
class carla_apis():
    ...
    def construct_acc(self):
        # check if ego vehicle exists
        if self.ego_vehicle is None:
            print("no ego vehicle exists")
            return
        
        # construct acc
        # spawn vehicle in front of ego vehicle
        vehicle_bp = self.blueprint_library.find('vehicle.audi.etron')
        spawn_points = self.world.get_map().get_spawn_points()
        spawn_point = spawn_points[SPAWN_POINT_NUMBER]
        # get the transform of ego vehicle
        ego_transform = self.ego_vehicle.get_transform()
        # set the spawn point of acc vehicle
        # spawn_point.location.x = ego_transform.location.x + 10
        
        # avoid the collision
        spawn_point.location.y = ego_transform.location.y + 10
        spawn_point.location.z = ego_transform.location.z + 1

        self.acc_vehicle = self.world.spawn_actor(vehicle_bp, spawn_point)

        # acc vehicle takes the same action as the eco vehicle
        self.acc_vehicle.set_autopilot(True)
